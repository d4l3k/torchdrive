...
class Transform3d:
    ...
    def __init__(
        ...
        if matrix is None:
            self._matrix: torch.Tensor = torch.eye(4, dtype=dtype, device=device).view(
                1, 4, 4
            )
        else:
        ...
        self._transforms: List[Transform3d] = []  # store transforms to compose
        # self._lu = None
        self.device = device
        self.dtype = dtype
    ...
    # pyre-fixme[2]: no type specified
    ...
    def to(
        ...
        Args:
          device: Device for the new tensor.

...
class DeformableTransformer(nn.Module):
    def __init__(
        ...
        if two_stage:
            self.enc_output: nn.Linear = nn.Linear(d_model, d_model)
            self.enc_output_norm: nn.LayerNorm = nn.LayerNorm(d_model)
            self.pos_trans: nn.Linear = nn.Linear(d_model * 2, d_model * 2)
            self.pos_trans_norm: nn.LayerNorm = nn.LayerNorm(d_model * 2)
        else:
            self.reference_points: nn.Linear = nn.Linear(d_model, 2)
    ...
    def _reset_parameters(self) -> None:
        ...
        for m in self.modules():
            if isinstance(m, MSDeformableAttention2d):
                m._reset_parameters()
    ...
    def get_proposal_pos_embed(self, proposals: torch.Tensor) -> torch.Tensor:
        num_pos_feats = 128
        temperature = 10000
        scale = 2 * math.pi
    ...
    def forward(
        ...
        if self.two_stage:
            return (
                ...
                # pyre-fixme[61]: not always defined
                enc_outputs_class,
                # pyre-fixme[61]: not always defined
                enc_outputs_coord_unact,
            )
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        # self attention
        self.self_attn = MSDeformableAttention2d(d_model, n_levels, n_heads, n_points)
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)

        # ffn
        self.linear1 = nn.Linear(d_model, d_ffn)
        self.activation: Callable[[torch.Tensor], torch.Tensor] = _get_activation_fn(
            activation
        )
